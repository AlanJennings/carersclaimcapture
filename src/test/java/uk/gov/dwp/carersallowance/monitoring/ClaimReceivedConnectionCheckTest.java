...
public class ClaimReceivedConnectionCheckTest {
    private ClaimReceivedConnectionCheck claimReceivedConnectionCheck;
    ...
    public void setUp() throws Exception {
        claimReceivedConnectionCheck = new ClaimReceivedConnectionCheck("c3", "4.00", "http:/localhost:9009", restTemplate);
    }
    ...
    public void testCheckUnhealthy() throws Exception {
        ...
        thenCheckShouldReturn(CADSHealthCheck.Result.unhealthy("Claim received ping failed: 503."));
    }
    ...
    private void thenCheckShouldReturn(final CADSHealthCheck.Result result) {
        assertThat("connection check matches expected result", claimReceivedConnectionCheck.check(), is(result));
    }

    private void thenCheckShouldReturn(final String result) {
        assertThat("connection check matches expected result", claimReceivedConnectionCheck.check().toString(), is(result));
    }
}